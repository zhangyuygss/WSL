    ...
    for bs in range(batch_size):
        ...
        for cls_idx in indexes:
            ...
            with warnings.catch_warnings():
                ...
                cls_map = imtrans.resize(cls_map, img_sz, order=3, mode='constant')
                cls_map_norm = imtrans.resize(cls_map_norm, img_sz, order=3, mode='constant')
...
def spn_torch_rst(batch_names, targets, img_szs, att_map_dir, img_list_file):
    '''
    Get bbox localiztion result for spn torch version from 
    hdf5 file generated before
    '''
    batch_size, cls_number = targets.shape
    ...
    for bs in range(batch_size):
        img_name = batch_names[bs]
        atten_maps = h5py.File(os.path.join(att_map_dir,
                                            '{}.h5'.format(img_name.split('.')[0])))
        img_sz = img_szs[bs, :]
        map_save_dir = os.path.join(att_map_dir, img_name.split('.')[0])
        os.makedirs(map_save_dir, exist_ok=True)
        img_data = (img_name,)
        ...
        for cls_idx in indexes:
            cls[cls_idx] = 1
            cls_map = atten_maps['{}'.format(cls_idx+1)].value
            cls_map_norm = cv2.normalize(cls_map, None, 0.0, 0.99, cv2.NORM_MINMAX)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                # cls_map_norm = imtrans.resize(cls_map_norm, img_sz, order=3, mode='constant')
                # cls_map = imtrans.resize(cls_map, img_sz, order=3, mode='constant')
                cls_map_img = skimage.img_as_float(cls_map_norm)
                cls_map_file = map_save_dir + '/' + img_name.split('.')[0] + \
                               '_{}.png'.format(cls_idx)
                imio.imsave(cls_map_file, cls_map_img)
            box = gen_bbox(cls_map)
            cord = np.array([cls_idx, box[0], box[1], box[2], box[3]])
            cords = cord if cords is None else np.hstack((cords, cord))
        img_data += tuple(cls)
